...
int fn_NetSim_LTE_CalculateRxPower(NETSIM_ID enbId,NETSIM_ID enbInterface,LTE_ASSOCIATEUE_INFO* info,unsigned int carrier_index)
{
	...
	dRxPower_DL = dTXPower_DL + dGainTxr + dGainRver - fA1dB - fA2dB - PL_data[i]->ATTENUATION;
	dRxPower_UL = dTXPower_UL + dGainTxr + dGainRver - fA1dB - fA2dB - PL_data[i]->ATTENUATION;
	flag++;
		}
	}
	if (flag == 0)
